...
from integrations.hubspot import create_integration_item_metadata_object
...
@app.get('/integrations/hubspot/test-authorize')
async def test_authorize_hubspot(user_id: str, org_id: str):
...
@app.post("/integrations/hubspot/test-fetch")
async def test_fetch_items(user_id: str = Form(...), org_id: str = Form(...)):
...
@app.post("/integrations/hubspot/test-format")
async def test_format(user_id: str = Form(...), org_id: str = Form(...)):
    try:
        print("🔁 Starting /test-format")
        ...
        print("📦 Raw items from HubSpot:", items)

        formatted = await create_integration_item_metadata_object(items)
        print("✅ Formatted result:", formatted)

        return formatted
    
    except Exception as e:
        print("❌ ERROR in /test-format:", str(e))
...
async def load_hubspot_items(credentials: str = Form(...)):
    ...
    return await get_items_hubspot(parsed_credentials)